...
class OrmRead:
    ...
    @staticmethod
    def list_all(engine: create_engine, cls: Type[OrmDomain]) -> List[OrmDomain]:
        with Session(engine) as session:
            domain = cls()  # Instantiate the domain class
            mappings = domain.resolve_mappings()

            select_statement = text(f"SELECT * FROM {domain.table_name()}")
            return OrmRead.list_and_merge(session, mappings, select_statement, OrmRead.NO_PARAMS)

    @staticmethod
    def list(engine: create_engine, cls: Type[OrmDomain], select_statement: str, set_params_fn: Callable[[str], str]) -> List[OrmDomain]:
        with Session(engine) as session:
            domain = cls()  # Instantiate the domain class
            mappings = domain.resolve_mappings()

            return OrmRead.list_and_merge(session, mappings, text(select_statement), set_params_fn)

    @staticmethod
    def list_and_merge(session: Session, db_domain_field_mappings: List[OrmMapping], select_statement: str, set_params_fn: Callable[[str], str]) -> List[OrmDomain]:
        objs = []
        ...
        try:
            ...
            for row in statement:
                props = dict(row)
                obj = db_domain_field_mappings.to_domain(props)
                objs.append(obj)
